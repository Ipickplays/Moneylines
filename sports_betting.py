...
def fetch_historical_games(path):
    """Fetch historical game data from CSV, return empty DataFrame if missing or error."""
...
def build_team_stats(df):
    """Build team and head-to-head statistics from historical data."""
    team_stats, h2h_stats, h2h_history = {}, {}, {}
...
def train_model(df, team_stats, h2h_stats):
    """Train RandomForestClassifier on historical game data."""
    X, y = [], []
...
def fetch_odds(sport_key, target_day):
    """Fetch odds from API with retries."""
    for attempt in range(MAX_API_RETRIES):
        try:
            url = (f"https://api.the-odds-api.com/v4/sports/{sport_key}/odds/"
                   f"?apiKey={ODDS_API_KEY}&regions=us&markets=h2h&oddsFormat=american"
                   f"&dateFormat=iso&commenceTimeFrom={target_day}T00:00:00Z"
                   f"&commenceTimeTo={target_day}T23:59:59Z")
...
# --- Reset CSVs for offseason leagues ---
